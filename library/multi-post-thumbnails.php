...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			//add_filter("admin_{$this->id}_html', 'add_featured_image_options');
		}
		...
		public function thumbnail_meta_box() {
			global $post;
			$thumbnail_id = get_post_meta($post->ID, "_{$this->post_type}_{$this->id}_thumbnail_id", true);
			echo $this->post_thumbnail_html($thumbnail_id);
			/*if($this->extra_options) { 
				call_user_func($this->extra_options);
			}*/
		}
		 ...
		 */
		public static function the_post_thumbnail($post_type, $thumb_id, $post_id = null, $size = 'full', $attr = '', $link_to_original = false) {
		 ...
		 * @param string|array $attr Optional. Query string or array of attributes.
		  */
		public static function get_the_post_thumbnail($post_type, $thumb_id, $post_id = NULL, $size = 'full', $attr = '' , $link_to_original = false) {
			global $id;
		 ...
		 * @param string|array $attr Optional. Query string or array of attributes.
		  */
		public static function get_the_post_thumbnail_url($post_type, $thumb_id, $post_id = NULL, $size = 'full') {
			global $id;
			$post_id = (NULL === $post_id) ? $id : $post_id;
			$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
			$size = apply_filters("{$post_type}_{$post_id}_thumbnail_size", $size);
		...
		private function post_thumbnail_html($thumbnail_id = NULL) {
			...
			if($this->extra_options) {
				$function = $this->extra_options;
				$content = '<div class="project-slider-img-upload">' . $content . '</div>';
				$content .= call_user_func( $function, $post_ID );
			}
		...
		public function set_thumbnail() {
			...
			if ($thumbnail_id == '-1') {
				delete_post_meta($post_ID, "_{$this->post_type}_{$this->id}_thumbnail_id");
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					update_post_meta($post_ID, "_{$this->post_type}_{$this->id}_thumbnail_id", $thumbnail_id);
